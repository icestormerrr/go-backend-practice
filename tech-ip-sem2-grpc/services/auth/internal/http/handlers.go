...
func (h *Handler) VerifyHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		writeJSON(w, http.StatusMethodNotAllowed, errorResponse{Error: "method not allowed"})
		return
	}

	token, ok := service.ParseBearerToken(r.Header.Get("Authorization"))
	if !ok {
		writeJSON(w, http.StatusUnauthorized, verifyResponse{Valid: false, Error: "unauthorized"})
		return
	}

	subject, valid := h.service.VerifyToken(r.Context(), token)
	if !valid {
		writeJSON(w, http.StatusUnauthorized, verifyResponse{Valid: false, Error: "unauthorized"})
		return
	}

	h.logger.Printf("request_id=%s verify_http subject=%s", middleware.GetRequestID(r.Context()), subject)
