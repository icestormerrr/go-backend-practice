...
func NewHandler(service *service.Service, authClient *grpcclient.Client, logger *log.Logger) *Handler {
	return &Handler{service: service, authClient: authClient, logger: logger}
}
...
func (h *Handler) authorize(w http.ResponseWriter, r *http.Request) bool {
	...
	statusCode := grpcclient.HTTPStatus(err)
	switch statusCode {
	case http.StatusUnauthorized:
		writeJSON(w, statusCode, errorResponse{Error: "unauthorized"})
	case http.StatusServiceUnavailable:
		writeJSON(w, statusCode, errorResponse{Error: "authorization service unavailable"})
	default:
		writeJSON(w, statusCode, errorResponse{Error: "authorization service error"})
	}
